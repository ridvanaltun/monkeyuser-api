...
const comic = async (req: NowRequest, res: NowResponse) => {
  const { id } = req.query
  ...
  try {
    ...
    if (id > comicsCount || id <= 0) {
      return res.status(404).json({
        detail: "Comic strip not found."
      });
    }

    // comic
    const comic = comics[comicsCount - id]
...
export default allowCors(comic)
